...
async function saveSettings() {
  ...
  if (trimmedBinary !== (instance.value.binary ?? '')) patch.binary = trimmedBinary
  if (memoryInput.value !== null && memoryInput.value !== instance.value.memory)
  ...
  if (gamePortInput.value !== null && gamePortInput.value !== (instance.value.ports?.game ?? 25565))
    patch.game_port = gamePortInput.value
  if (rconPortInput.value !== null && rconPortInput.value !== (instance.value.ports?.rcon ?? 25575))
...
async function handleUpgrade() {
  ...
  const response = await upgradeInstance(instance.value.id, instance.value.vendor, upgradeVersion.value)
  if (!response) { upgrading.value = false; return }
  emit('instanceUpdated', response)
  message.success(`Upgraded to ${instance.value.vendor} ${upgradeVersion.value}`)
    ...
    <section class="space-y-5">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <n-input-number v-model:value="gamePortInput" :min="1" :max="65535" :disabled="isRunning" class="w-full"
            placeholder="25565" />
        </div>
        <div>
          <div class="text-sm text-neutral-400 mb-1.5">RCON port</div>
          <n-input-number v-model:value="rconPortInput" :min="1" :max="65535" :disabled="isRunning" class="w-full"
            placeholder="25575" />
        </div>
      </div>
      <n-button type="primary" :loading="saving" :disabled="isRunning" @click="saveSettings">Save settings</n-button>
