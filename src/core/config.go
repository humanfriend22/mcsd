...
func LoadConfig() (*Config, error) {
	...
	if err != nil {
		...
		return nil, &InternalError{Message: fmt.Sprintf("open global config: %s", err.Error())}
	}
	defer file.Close()

	var config Config
	if err := json.NewDecoder(file).Decode(&config); err != nil {
		return nil, &InternalError{Message: fmt.Sprintf("decode global config: %s", err.Error())}
	}

	total, err := TotalSystemMemory()
	if err != nil {
		return nil, &InternalError{Message: fmt.Sprintf("fetch total system memory: %s", err.Error())}
	}
...
func WriteConfig(config *Config) error {
	if err := os.MkdirAll(filepath.Dir(GlobalConfigPath), 0755); err != nil {
		return &InternalError{Message: fmt.Sprintf("create config dir: %s", err.Error())}
	}
	data, err := json.MarshalIndent(config, "", "  ")
	if err != nil {
		return &InternalError{Message: fmt.Sprintf("encode config: %s", err.Error())}
	}
	return WriteFile(GlobalConfigPath, append(data, '\n'), 0644)
}
